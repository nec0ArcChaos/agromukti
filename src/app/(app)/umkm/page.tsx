  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-neutral-900">Produk UMKM</h1>
          <p className="text-sm text-neutral-500">Produk olahan warga, mis. Wajik Tomat dari hasil panen desa.</p>
        </div>
            ...
            <tbody>
              {rows.map((p) => (
                <tr key={p.id}>
                  ...
                  <td>{p.nama}<br /><span className="text-xs text-neutral-400">{p.deskripsi ?? ""}</span></td>
                  <td className="text-xs text-neutral-500">{p.kategori ?? "-"}</td>
              ...
              {rows.length === 0 && <tr><td colSpan={6} className="py-6 text-center text-neutral-400">Belum ada produk UMKM.</td></tr>}
            </tbody>
          </table>
        )}
      </div>
